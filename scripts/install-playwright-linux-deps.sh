...
cat <<'EOF'
SpectralTV on Linux uses Playwright's official Docker image for weather channels.
...
On the first weather tune, SpectralTV starts container:
  spectraltv-playwright-chromium

Manual pull (optional):
  docker pull mcr.microsoft.com/playwright:v1.49.0-jammy

Check container:
  docker logs spectraltv-playwright-chromium
