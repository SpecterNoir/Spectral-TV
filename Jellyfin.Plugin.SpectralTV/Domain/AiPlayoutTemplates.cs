...
public static class AiPlayoutTemplates
{
    ...
    public static string BuildPromptSection(AiPlayoutTemplate template)
    {
        ...
        if (template.Id is "movie-marathon" or "holiday-channel")
        {
            ...
            lines.Add("- SpectralTV repacks movies first in release chronological order (earliest year/date first), then other catalog items.");
        }
    ...
    private static string BuildSeriesEpisodeBlockingSection(AiPlayoutTemplate template)
    {
        ...
        return string.Join('\n', new[]
        {
            ...
            "- For TV series, use consecutive slots with the same jellyfinItemId; SpectralTV plays the next episode in order for each consecutive slot.",
