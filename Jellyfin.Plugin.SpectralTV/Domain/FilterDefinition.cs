...
{
    /// <summary>
    /// SpectralTV preset identifier (e.g. spectraltv-retro). Used for channel rules, not Jellyfin tag queries.
    /// </summary>
    ...
    public static FilterDefinition? Parse(string? filterJson)
    {
        ...
        try
        {
            return SpectralTvJson.Deserialize<FilterDefinition>(filterJson);
        }
        catch
        {
            return null;
    ...
    public static bool IsFintvChannelTag(string? tag)
        => !string.IsNullOrWhiteSpace(tag)
           && tag.StartsWith("spectraltv-", StringComparison.OrdinalIgnoreCase);
    ...
    public static string? ExtractFintvLibraryTag(string? filterJson)
    {
        ...
        return filter.Tags?
            .FirstOrDefault(tag => tag.StartsWith("spectraltv-", StringComparison.OrdinalIgnoreCase));
    }

    /// <summary>
    /// Optional Jellyfin tags from filter JSON (excludes SpectralTV preset identifiers).
    /// </summary>
