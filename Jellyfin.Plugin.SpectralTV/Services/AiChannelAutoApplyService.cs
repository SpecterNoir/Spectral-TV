...
public class AiChannelAutoApplyService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            SpectralTvDebugLog.Ai(_logger, "Auto-apply skipped for {ChannelId}: AI disabled or auto-apply off", channelId);
            return AiAutoApplyChannelResult.Skipped("AI auto-apply is disabled.");
        }

        SpectralTvDebugLog.Ai(_logger, "Auto-apply starting for channel {ChannelId}", channelId);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            SpectralTvDebugLog.Ai(_logger, "Channel {ChannelName} ({ChannelId}) is not eligible for AI lineups", channel.Name, channelId);
            return AiAutoApplyChannelResult.Skipped($"{channel.Name} is not eligible for AI lineups.");
        }

        SpectralTvDebugLog.Ai(
            _logger,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (existing.Count == 0)
        {
            SpectralTvDebugLog.Ai(_logger, "No future playout to clear for channel {ChannelId}", channelId);
            return;
        }

        SpectralTvDebugLog.Ai(_logger, "Clearing {Count} future playout items for channel {ChannelId}", existing.Count, channelId);
