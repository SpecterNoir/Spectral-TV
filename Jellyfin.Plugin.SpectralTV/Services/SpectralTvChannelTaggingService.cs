...
/// <summary>
/// Applies spectraltv-* Jellyfin tags to library items using built-in channel rules.
/// </summary>
public class SpectralTvChannelTaggingService
{
    ...
    private readonly ILogger<SpectralTvChannelTaggingService> _logger;

    public SpectralTvChannelTaggingService(
        ILibraryManager libraryManager,
        JellyfinCatalogService catalog,
        ILogger<SpectralTvChannelTaggingService> logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _logger.LogInformation(
                "SpectralTV channel auto-tagging started: {Count} item(s), fullRetag={FullRetag}.",
                items.Count,
                fullRetag);
            ...
            for (var index = 0; index < items.Count; index++)
            {
                ...
                {
                    _logger.LogWarning(ex, "SpectralTV auto-tagging skipped item {ItemId} ({Name}).", item.Id, item.Name);
                    state.SkippedItems++;
                }
            ...
            progress?.Report(100);
            _logger.LogInformation(
                "SpectralTV channel auto-tagging finished: processed={Processed}, tagged={Tagged}, skipped={Skipped}.",
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "SpectralTV channel auto-tagging failed.");
            throw;
        }
        finally
        {
