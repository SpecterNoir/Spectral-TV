...
/// Linux uses the spectraltv-playwright-chromium Docker sidecar over CDP (Windows launches local Chromium).
/// </summary>
public class PlaywrightRuntimeService
{
        ...
        if (OperatingSystem.IsLinux())
        {
            ...
            if (PlaywrightDockerBrowserService.RunsInsideDocker())
            {
                throw new InvalidOperationException(
                    "Docker is required for SpectralTV weather channels when Jellyfin runs in a container. Mount "
            ...
            throw new InvalidOperationException(
                "Docker is required for SpectralTV weather channels on Linux. Mount /var/run/docker.sock into Jellyfin "
    ...
    private void ConfigureEnvironment()
    {
        ...
        var plugin = Plugin.Instance
            ?? throw new InvalidOperationException("SpectralTV plugin is not initialized.");
    ...
    private async Task EnsureChromiumInstalledAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Ensuring Playwright Chromium is installed for SpectralTV weather channels");
            ...
            _browsersInstalled = true;
            _logger.LogInformation("Playwright Chromium is ready for SpectralTV weather channels");
        }
        finally
        {
            InstallLock.Release();
