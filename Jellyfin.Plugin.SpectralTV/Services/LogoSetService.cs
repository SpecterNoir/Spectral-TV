...
{
    public const string Binarygeek119GitHubRef = "spectraltv2";

    public const string Binarygeek119GitHubTreeUrl =
        "https://api.github.com/repos/binarygeek119/open-channel-logos/git/trees/spectraltv2?recursive=1";

    public const string Binarygeek119GitHubRawBase =
        "https://raw.githubusercontent.com/binarygeek119/open-channel-logos/spectraltv2/";
    ...
    public LogoSetService(SpectralTvDbContext db, IHttpClientFactory httpClientFactory, ILogger<LogoSetService> logger)
    {
        _db = db;
    ...
    internal static ChannelPresetDefinition? FindPresetForChannel(Channel channel)
    {
        ...
        {
            return ChannelPresets.All.FirstOrDefault(p => p.Id == "spectraltv-weatherstar4000")
                ?? ChannelPresets.All.FirstOrDefault(p => p.Id == "spectraltv-news");
        }
    ...
    private HttpClient CreateGitHubClient()
    {
        ...
        client.DefaultRequestHeaders.UserAgent.ParseAdd("SpectralTV-Jellyfin-Plugin");
