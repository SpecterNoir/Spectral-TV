...
public class AiLineupAutoApplyTask : IScheduledTask
{
    ...
    public string Name => "SpectralTV AI Lineup Auto-Apply (Channel Add)";

    public string Key => "SpectralTVAiLineupAutoApply";
    ...
    public async Task ExecuteAsync(IProgress<double> progress, CancellationToken cancellationToken)
    {
        ...
        if (!settings.Enabled)
        {
            _logger.LogInformation("SpectralTV AI lineup auto-apply skipped because AI is disabled.");
            progress.Report(100);
            return;
        }
        ...
        _logger.LogInformation(
            "SpectralTV AI lineup task finished: {Processed} queued channel(s), {Extended} horizon extension(s).",
            processed,
            extended);
    }
}
