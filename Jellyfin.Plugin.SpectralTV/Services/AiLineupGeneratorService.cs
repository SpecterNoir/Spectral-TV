...
{
    private static readonly JsonSerializerOptions JsonOptions = SpectralTvJson.Options;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException(
                "Invalid schedule time zone in SpectralTV settings. Set Dashboard → Plugins → SpectralTV → schedule time zone to a valid IANA id (e.g. America/New_York).",
                ex);
        }
        catch (Exception ex)
        {
    ...
    private static string BuildSystemPrompt(ChannelCatalogMode catalogMode, AiPlayoutTemplate playoutTemplate)
    {
        ...
        return """
            You are a TV channel scheduling assistant for SpectralTV.
