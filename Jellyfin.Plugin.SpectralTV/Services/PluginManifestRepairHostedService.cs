...
public sealed class PluginManifestRepairHostedService : IHostedService
{
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning(ex, "SpectralTV could not repair plugin manifest image settings.");
        }
    ...
    private void RepairManifest()
    {
        ...
        if (changed)
        {
            ...
            _logger.LogInformation("SpectralTV plugin manifest now uses embedded plugin image resource.");
        }
        ...
        {
            _logger.LogDebug(ex, "SpectralTV could not delete disk plugin logo {LogoPath}; it may be in use.", diskLogo);
        }
    }
}
