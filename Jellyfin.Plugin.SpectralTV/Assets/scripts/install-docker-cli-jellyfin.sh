...
# Bundled with SpectralTV at: plugins/SpectralTV_<version>/scripts/install-docker-cli-jellyfin.sh
#
# Preferred: run from the Docker HOST (where Jellyfin runs):
#   bash /path/to/config/plugins/SpectralTV_<version>/scripts/install-docker-cli-jellyfin.sh jellyfin
#
# If you are already inside the Jellyfin container as root:
#   bash /config/plugins/SpectralTV_<version>/scripts/install-docker-cli-jellyfin.sh --inside-container
#
...
verify_docker_cli() {
  if docker version >/dev/null 2>&1; then
    ...
    echo
    echo "Done. SpectralTV Weather Docker buttons should work after restarting Jellyfin if needed."
    return 0
  fi
