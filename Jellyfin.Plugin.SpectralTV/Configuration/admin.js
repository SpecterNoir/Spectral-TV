(function () {
    ...
    function candidateSummary(candidate) {
        ...
        const finTvListId = candidate.finTvListId || candidate.SpectralTvListId;
    ...
    async function ensureSpectralTvLists(force) {
    ...
    async function saveChannel(e) {
        ...
        if (!configPage) {
            toast('SpectralTV admin page is not ready. Close and reopen SpectralTV settings.', 'error');
            return;
        }
    ...
    function openSlotEditor(index) {
        ...
        const body = `
            ...
            <div class="field">
                ...
                <select id="slot-add-kind" class="emby-select">
                    ...
                    <option value="3">SpectralTV list</option>
                </select>
            </div>
        ...
        function renderAddPanel() {
            ...
            if (kind === 0) {
                ...
                ensureSpectralTvLists().then((lists) => {
                    panel.innerHTML = `<label class="field"><span>SpectralTV list</span>
    ...
    function normalizeConfigPageRoot(node) {
        ...
        if (node.id === 'SpectralTVConfigPage') {
            return node;
        }

        if (typeof node.querySelector === 'function') {
            const nested = node.querySelector('#SpectralTVConfigPage');
            if (nested) {
                return nested;
            }
        }
        ...
        if (typeof node.closest === 'function') {
            return node.closest('#SpectralTVConfigPage');
        }
    ...
    async function loadLists() {
        try {
            await ensureSpectralTvLists(true);
            renderListsTable();
        } catch (err) {
            reportApiError(err, 'Could not load SpectralTV lists.');
        }
    }
    ...
    function renderListsTable() {
        ...
        if (!finTvLists.length) {
            el.innerHTML = '<div class="empty-state">No SpectralTV lists registered yet. Add a Jellyfin playlist to use it in lineups.</div>';
            return;
        }
    ...
    async function openListForm(editId) {
        ...
        openModal(existing ? 'Edit SpectralTV List' : 'Add SpectralTV List', body, `
    ...
    async function deleteList(id) {
        if (!confirm('Remove this SpectralTV list registration?')) return;
        try {
    ...
    async function openSpecialPresentationForm(editId) {
        ...
        const body = `
            ...
            <label class="field"><span>Content mode</span>
                <select id="sp-content-mode" class="emby-select">
                    ...
                    <option value="2">SpectralTV list</option>
                </select></label>
        ...
        document.getElementById('sp-save').onclick = async () => {
            ...
            if (mode === 1) {
                ...
                if (!listId) {
                    toast('Select a SpectralTV list.', 'error');
                    return;
                }
    ...
    function switchTab(name) {
        ...
        qa('.spectraltv-tabs .tab').forEach((t) => t.classList.toggle('active', t.dataset.tab === name));
    ...
    function bindEvents() {
        ...
        qa('.spectraltv-tabs .tab').forEach((tab) => tab.onclick = () => switchTab(tab.dataset.tab));
    ...
    function init(page) {
        ...
        bindEvents();
        startOnAirPolling();
        return refresh().catch((err) => reportApiError(err, 'Could not load SpectralTV admin.'));
    }
    ...
    function resolveConfigPage(preferred) {
        ...
        const pages = document.querySelectorAll('#SpectralTVConfigPage');
    ...
    function bootSpectralTvAdmin(page) {
        page = resolveConfigPage(page);
        if (!page || !window.SpectralTV || !window.SpectralTV.init) {
            return false;
        }

        window.SpectralTV.init(page);
        return true;
    }

    window.SpectralTV = { init, refresh, loadChannels, loadSetup, bootSpectralTvAdmin };
})();

export default function (view) {
    function boot() {
        if (window.SpectralTV && window.SpectralTV.bootSpectralTvAdmin) {
            window.SpectralTV.bootSpectralTvAdmin(view);
        }
    }

    view.addEventListener('viewshow', boot);
    view.addEventListener('viewdestroy', function () {
        delete view.dataset.spectraltvBound;
        document.querySelectorAll('#SpectralTVConfigPage').forEach((page) => {
            delete page.dataset.spectraltvBound;
        });
    });
    boot();
}
