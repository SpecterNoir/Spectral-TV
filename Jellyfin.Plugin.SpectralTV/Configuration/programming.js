(function () {
    ...
    async function api(path, options) {
        options = options || {};
        const url = resolveUrl('SpectralTV/api' + (path.startsWith('/') ? path : '/' + path));
        ...
        if (!response.ok) {
            ...
            throw new Error(parseError(text, response.statusText));
        }
    ...
    function setNewChannelPanel(open) {
        const panel = $('vtv-new-channel-panel');
        if (!panel) return;
        panel.classList.toggle('show', !!open);
        if (open) $('vtv-new-channel-name')?.focus();
    }

    function suggestChannelNumber() {
        const values = channels.map((c) => Number(c.number)).filter((n) => Number.isFinite(n));
        return values.length ? Math.ceil(Math.max.apply(null, values)) + 1 : 101;
    }
    ...
    async function loadChannels(preferredId) {
        ...
        if (previous && channels.some((c) => c.id === previous)) select.value = previous;

        const hasChannels = channels.length > 0;
        if ($('vtv-rebuild')) $('vtv-rebuild').disabled = !hasChannels;
        if ($('vtv-delete-channel')) $('vtv-delete-channel').disabled = !hasChannels;
        if ($('vtv-save-settings')) $('vtv-save-settings').disabled = !hasChannels;

        if (!hasChannels) {
            state = null;
            renderState();
            $('vtv-new-channel-number').value = String(suggestChannelNumber());
            setNewChannelPanel(true);
            showStatus('No virtual channels exist yet. Create your first channel above.', 'warn');
            return;
        }
    ...
    function renderPrograms() {
        ...
        totalEl.textContent = items.length ? `Enabled airtime targets total ${rounded}%. Values are normalized automatically.` : 'No programming sources yet.';
        ...
        body.innerHTML = items.map((source) => `
            <tr data-source-id="${source.id}">
                <td><strong>${escapeHtml(source.name)}</strong></td><td>${escapeHtml(source.type)}</td>
    ...
    function renderFillers() {
        ...
        body.innerHTML = items.map((source) => `
            <tr data-filler-id="${source.id}">
                ...
                <td><select data-role="kind" class="emby-select"><option value="0"${Number(source.kind) === 0 ? ' selected' : ''}>Promo</option><option value="1"${Number(source.kind) === 1 ? ' selected' : ''}>Bumper</option><option value="2"${Number(source.kind) === 2 ? ' selected' : ''}>Commercial</option><option value="3"${Number(source.kind) === 3 ? ' selected' : ''}>Station ID</option></select></td>
    ...
    async function createChannel() {
        const name = ($('vtv-new-channel-name').value || '').trim();
        const number = Number($('vtv-new-channel-number').value || 0);
        if (!name) return showStatus('Enter a channel name.', 'warn');
        if (!Number.isFinite(number) || number <= 0) return showStatus('Enter a valid channel number.', 'warn');
        const created = await api('/channels', { method: 'POST', body: {
            number, name, enabled: true, contentType: 0,
            aspectRatio: Number($('vtv-new-channel-aspect').value || 0),
            scanlinesEnabled: false, bugPlacement: 0, logoSetId: null,
            logoFileName: null, audioLanguage: 'eng', weatherLocationQuery: null
        }});
        if (!created?.id) throw new Error('Channel was created but Jellyfin did not return its ID.');
        await api('/programming/' + created.id + '/settings', { method: 'PUT', body: {
            channelId: created.id, enabled: true, fillerEnabled: true, fillerChancePercent: 100,
            minFillerItems: 1, maxFillerItems: 2, maxFillerSeconds: 180, fillerRepeatWindow: 12
        }});
        $('vtv-new-channel-name').value = '';
        setNewChannelPanel(false);
        await loadChannels(created.id);
        showStatus(`Channel “${name}” created. Add programming below, then rebuild it.`, 'good');
    }
    ...
    async function deleteChannel() {
        ...
        if (!window.confirm(`Delete ${name}? This removes only the virtual channel and its schedule; it does not delete any Jellyfin media.`)) return;
        await api('/channels/' + encodeURIComponent(id), { method: 'DELETE' });
        state = null;
        await loadChannels();
        showStatus(`Virtual channel “${name}” deleted.`, 'good');
    }
    ...
    async function saveSettings() {
        ...
        if (max < min) return showStatus('Maximum filler items must be at least the minimum.', 'warn');
        await api('/programming/' + id + '/settings', { method: 'PUT', body: {
            channelId: id, enabled: $('vtv-enabled').checked,
            fillerEnabled: $('vtv-filler-enabled').checked,
            fillerChancePercent: Number($('vtv-filler-chance').value || 0),
            minFillerItems: min, maxFillerItems: max,
    ...
    async function searchCatalog(query, resultsId, onPick) {
        ...
        results.innerHTML = (data || []).map((item) => `
            <div class="vtv-result" data-id="${item.id}" data-type="${escapeHtml(item.type)}"><strong>${escapeHtml(item.name)}</strong><div class="vtv-sub">${escapeHtml(item.type)}${item.year ? ' · ' + item.year : ''}${item.runtimeMinutes ? ' · ' + item.runtimeMinutes + ' min' : ''}</div></div>`).join('') || '<div class="vtv-result vtv-muted">No matches</div>';
        results.querySelectorAll('.vtv-result[data-id]').forEach((row) => { row.onclick = () => onPick(row.dataset.id, row.dataset.type); });
    }

    async function addProgram(itemId) {
        const channelId = currentChannelId();
        if (!channelId) return showStatus('Create or select a channel first.', 'warn');
        await api('/programming/' + channelId + '/sources', { method: 'POST', body: {
            jellyfinItemId: itemId, targetAirtimePercent: Number($('vtv-new-share').value || 1),
            playbackMode: Number($('vtv-new-mode').value || 0), enabled: true
        }});
        $('vtv-program-search').value = ''; $('vtv-program-results').innerHTML = '';
        showStatus('Programming source added.', 'good');
        await loadSelectedChannel();
    }

    async function saveProgramSource(row, id) {
        await api('/programming/sources/' + id, { method: 'PUT', body: {
        ...
        showStatus('Programming source updated.', 'good'); await loadSelectedChannel();
    }
    ...
    async function removeProgramSource(id) {
        ...
        showStatus('Programming source removed.', 'good'); await loadSelectedChannel();
    }

    async function addFiller(itemId, type) {
        if (type === 'Series' || type === 'Season') return showStatus('Promos and bumpers must be individual playable media items, not a series or season.', 'warn');
        const channelId = currentChannelId();
        if (!channelId) return showStatus('Create or select a channel first.', 'warn');
        await api('/programming/' + channelId + '/fillers', { method: 'POST', body: {
            jellyfinItemId: itemId, kind: Number($('vtv-new-filler-kind').value || 0),
            weight: Number($('vtv-new-filler-weight').value || 1), enabled: true
        }});
        $('vtv-filler-search').value = ''; $('vtv-filler-results').innerHTML = '';
        showStatus('Promo / filler item added.', 'good'); await loadSelectedChannel();
    }

    async function saveFillerSource(row, id) {
        await api('/programming/fillers/' + id, { method: 'PUT', body: {
            kind: Number(row.querySelector('[data-role="kind"]').value || 0),
            weight: Number(row.querySelector('[data-role="weight"]').value || 1),
            enabled: row.querySelector('[data-role="enabled"]').checked
        }});
        showStatus('Promo / filler source updated.', 'good'); await loadSelectedChannel();
    }
    ...
    async function removeFillerSource(id) {
        ...
        showStatus('Promo / filler source removed.', 'good'); await loadSelectedChannel();
    }
    ...
    async function rebuildChannel() {
        ...
        showStatus('Channel rebuild started…'); pollRebuild(channelId, 0);
    }
    ...
    async function pollRebuild(channelId, attempt) {
        ...
        try {
            ...
            if (name === 'completed') { showStatus(`Rebuild complete. ${status.playoutItemCount || 0} future playout items are ready.`, 'good'); await loadSelectedChannel(); return; }
            if (name === 'failed') return showStatus('Rebuild failed: ' + (status.error || 'Unknown error'), 'warn');
            showStatus('Channel rebuild ' + name + '…'); window.setTimeout(() => pollRebuild(channelId, attempt + 1), 1500);
        } catch (err) { showStatus(err.message, 'warn'); }
    }

    function bindSearch(inputId, resultsId, onPick) {
        const input = $(inputId); if (!input) return;
        input.oninput = () => {
            window.clearTimeout(searchTimers[inputId]);
            searchTimers[inputId] = window.setTimeout(() => searchCatalog(input.value, resultsId, onPick).catch((err) => showStatus(err.message, 'warn')), 250);
        };
    }

    function bindEvents() {
        $('vtv-channel').onchange = () => loadSelectedChannel().catch((err) => showStatus(err.message, 'warn'));
        $('vtv-new-channel-toggle').onclick = () => { const panel = $('vtv-new-channel-panel'); const opening = !panel.classList.contains('show'); if (opening) $('vtv-new-channel-number').value = String(suggestChannelNumber()); setNewChannelPanel(opening); };
        $('vtv-cancel-new-channel').onclick = () => setNewChannelPanel(false);
        $('vtv-create-channel').onclick = () => createChannel().catch((err) => showStatus(err.message, 'warn'));
        $('vtv-delete-channel').onclick = () => deleteChannel().catch((err) => showStatus(err.message, 'warn'));
    ...
    async function init(view) {
        page = view || document.querySelector('#SpectralTVProgrammingPage');
        if (!page || page.dataset.vtvBound === '1') return;
        page.dataset.vtvBound = '1'; bindEvents();
        try { await loadChannels(); } catch (err) { showStatus(err.message, 'warn'); }
    }

    window.SpectralTVProgramming = { init };
})();

export default function (view) {
    function boot() { if (window.SpectralTVProgramming && window.SpectralTVProgramming.init) window.SpectralTVProgramming.init(view); }
    view.addEventListener('viewshow', boot);
    view.addEventListener('viewdestroy', function () { delete view.dataset.vtvBound; });
    boot();
}
